...
final class SnugArchiverTests: XCTestCase {
    ...
    func testDetectDMGFile() async throws {
        let archiver = try await SnugArchiver(
            storageURL: storageURL,
            hashAlgorithm: "sha256"
        )
        ...
        let (chunkType, contentType) = archiver.detectChunkType(for: testFile, data: testData)
        
        XCTAssertEqual(chunkType, "disk-image")
        XCTAssertEqual(contentType, "application/x-apple-diskimage")
    }
    
    func testDetectISO9660File() async throws {
        let archiver = try await SnugArchiver(
            storageURL: storageURL,
            hashAlgorithm: "sha256"
        )
        ...
        XCTAssertEqual(chunkType, "disk-image")
        XCTAssertEqual(contentType, "application/x-iso9660-image")
    }
    
    func testDetectVHDFile() async throws {
        let archiver = try await SnugArchiver(
            storageURL: storageURL,
            hashAlgorithm: "sha256"
        )
        ...
        XCTAssertEqual(chunkType, "disk-image")
        XCTAssertEqual(contentType, "application/x-vhd")
    }
    
    func testDetectRegularFile() async throws {
        let archiver = try await SnugArchiver(
            storageURL: storageURL,
            hashAlgorithm: "sha256"
        )
        ...
        XCTAssertEqual(chunkType, "file")
        XCTAssertNil(contentType)
    }
    
    func testDetectRawIMGFile() async throws {
        let archiver = try await SnugArchiver(
            storageURL: storageURL,
            hashAlgorithm: "sha256"
        )
